...
#sudo ryu-manager  /home/bessala/Projet_memoire/wifi_monitor_v2.py --observe-links --ofp-tcp-listen-port 6634
...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        #self.logger.info('pkt_in\t%x',datapath.id)
        ...
        # #self.logger.info("zpaczket in %s %s %s %s", dpid, src, dst, in_port)
        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            if eth.ethertype == ether_types.ETH_TYPE_IP:
                ...
                # if ip:
                    # #self.logger.info('packet\t%s\t%s\t%d', srcip, dstip, ip.tos)
    ...
    def packet_out_handler(self, ev):
        self.logger.info('pkt_out\t%x', ev.msg.datapath.id)
...
##############new code
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if flow.priority == 10],
            ...
            #self.logger.info('data\t%s\t%x\t%x\t%s\t%s\t%x\t%d\t%d\t%d',self.fields['time'],self.fields['datapath'],self.fields['in-port'],self.fields['eth_src'],self.fields['eth_dst'],self.fields['out-port'],self.fields['total_packets'],self.fields['total_bytes'],stat.match['metadata'])
    ...
    def _port_stats_reply_handler(self, ev):
        ...
        for stat in sorted(body, key=attrgetter('port_no')):
            ...
            if ev.msg.datapath.id not in counts:
                counts[ev.msg.datapath.id] = {"rx": 0, "tx": 0}
            
            counts[ev.msg.datapath.id]["rx"] += stat.rx_packets
            counts[ev.msg.datapath.id]["tx"] += stat.tx_packets
        
        for id in counts:
            self.logger.info('pkt_count\t%x\t%d\t%d', id, counts[id]["rx"], counts[id]["tx"])
    ...
    def get_topology_data(self):
        ...
        json_dataTopology = json.dumps(dataTopology)

        with open('topology.json', 'w') as file:
            file.write(json_dataTopology)
