...
proj_location_Src = os.getcwd() + "/"
proj_location_dataset = os.getcwd().replace("src", "dataset/")
proj_location_Binary = os.getcwd().replace("src", "binary/")
## command to run ##
cmd = "sudo ryu run "+proj_location_Src+"wifi_monitor_v8.py --observe-links --ofp-tcp-listen-port 6634"
...
CONSUMPTION_PKT_IN = 0.3
CONSUMPTION_PKT_OUT = 0.1
...
class Flow:
    def __init__(self, time_start, datapath, inport, ethsrc, ethdst, outport, packets, bytes, tos):
    ...
    def updateforward(self, packets, bytes, curr_time, tos):
        self.tos=tos
    ...
    def updatereverse(self, packets, bytes, curr_time, tos):
        self.tos=tos
...
consommation={"1":100,"2":100,"3":100}
...
def printclassifier(model):
    ...
    for key,flow in flows.items():
        ...
        # if flow.datapath in ['1','2','3']:          
        #     if flow.tos == 0:#'share services'
        #         consommation[flow.datapath] -= 0.001    
        #     elif flow.tos == 1:#'dedicated services'
        #         consommation[flow.datapath] -= 0.002
        #     elif flow.tos == 2: #'real time services'
        #         consommation[flow.datapath] -= 0.003  
    ...
    for dpid in energy:
        energy[dpid] -= (packet_counts[dpid]["received"]*CONSUMPTION_PKT_IN + packet_counts[dpid]["sent"]*CONSUMPTION_PKT_OUT)
...
def run_ryu(p,traffic_type=None,f=None,model=None):
    ...
    while True:
        ...
        if out != '' and out.startswith(b'pkt_count'): 
            # dpid = out.split(b'\t')[1:][0]
            ...
            packet_counts[fields[0]] = {"received": int(fields[1]), "sent": int(fields[2])}
            energy[fields[0]] = 320_000
            

        # if out != '' and out.startswith(b'pkt_in'): 
        #     dpid = out.split(b'\t')[1:][0]

        #     if dpid not in packet_counts:
        #         packet_counts[dpid] = {"received": 0, "sent": 0}
        #         energy[dpid] = 320
            
        #     packet_counts[dpid]["received"] += 1

        # if out != '' and out.startswith(b'pkt_out'): 
        #     dpid = out.split(b'\t')[1:][0]
        ...
        if out != '' and out.startswith(b'data'): #when Ryu 'wifi_monitor_v2.py' script returns output
            ...
            if unique_id in flows.keys():
                flows[unique_id].updateforward(int(fields[6]),int(fields[7]),int(fields[0]),int(fields[8])) #update forward attributes with time, packet, and byte count
            else:
                ...
                if rev_unique_id in flows.keys():
                    flows[rev_unique_id].updatereverse(int(fields[6]),int(fields[7]),int(fields[0]),int(fields[8])) #update reverse attributes with time, packet, and byte count
                else:
                    flows[unique_id] = Flow(int(fields[0]), fields[1], fields[2], fields[3], fields[4], fields[5], int(fields[6]), int(fields[7]), int(fields[8])) #create new flow object
